// Package abstraction is the one entry point an application uses.
//
//	a, err := abstraction.Discover()
//	downloads := a.Download()
//	ui.Bind(a.Download().Jobs())
//
// Each layer can discover itself — download.Discover, logging.Auto,
// config.Load — and that shape is idiomatic Go and nothing else: a blank import
// of a tier package has no equivalent in C++ or Python. This root is the entry
// point that has a spelling in every language the layers are written in.
//
// # What it does not do
//
